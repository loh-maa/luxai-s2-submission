...
class Unit:
    ...
    def move_cost(self, game_state, direction):
        ...
        if target_pos[0] < 0 or target_pos[1] < 0 or target_pos[1] >= len(board.rubble) or target_pos[0] >= len(board.rubble[0]):
            ...
            # return None
            return self.unit_cfg.BATTERY_CAPACITY * 10
    ...
    def recharge(self, x, repeat=0, n=1):
        x = min(x, self.unit_cfg.BATTERY_CAPACITY)
    ...
    def __post_init__(self):
        self.loc = (self.pos[0], self.pos[1])
        self.weight = 10 if self.unit_type == 'HEAVY' else 1
    ...
    def get_power_weight(self):
        return self.weight + self.power / (1000 if self.is_heavy() else 50)
